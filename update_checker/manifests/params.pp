class update_checker::params {
  ...
  case $::osfamily {
    /(?i:RedHat)/: { 
        ...
        $update_checker_target_dir  = "/home/${update_checker_user}/.gdrive/notes/scripts/sh"
    ...
    /(?i:Debian)/: { 
        ...
        $update_checker_target_dir  = "/home/$update_checker_user/.gdrive/notes/scripts/csh"
        $update_checker_cron_dir    = '/etc/cron.daily'
    }
    /(?i:FreeBSD)/: { 
        $update_checker_script      = 'pkgng-check.bash'
        $update_checker_packages    = ['bash']
        $update_checker_target_dir  = "/home/$update_checker_user/.gdrive/notes/scripts/bash"
